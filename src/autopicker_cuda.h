...
*
*  Based on autopicker.h, authored by Sjors Scheres
*/

#ifndef AUTOPICKER_CUDA_H_
#define AUTOPICKER_CUDA_H_
#include "src/image.h"
...
private:
	...
	void peakSearch(const MultidimArray<double> &Mccf, const MultidimArray<double> &Mpsi, int iref, int skip_side, std::vector<RelionPeak> &peaks);
	...
	void prunePeakClusters(std::vector<RelionPeak> &peaks, int min_distance);


	// Only keep those peaks that are at the given distance apart from each other
	void removeTooCloselyNeighbouringPeaks(std::vector<RelionPeak> &peaks, int min_distance);


};


#endif /* AUTOPICKER_CUDA_H_ */
