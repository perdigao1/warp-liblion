...
namespace relion
{
	...
	#define FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY_8(v) \
		for (long int n=0; n < NZYXSIZE(v); n += 8)
	...
	*/
	...
	public:
		...
		virtual void coreAllocate()
		{
			...
			if (mmapOn)
			{
				REPORT_ERROR("MultidimArray::coreAllocate: Not supported in Windows port.");
			}
			else
			{
		 ...
		 *
		 */
		virtual void coreAllocateReuse()
		{
			...
			if (mmapOn)
			{
				REPORT_ERROR("MultidimArray::coreAllocateReuse: Not supported in Windows port.");
			}
			else
			{
				////cudaMallocHost((void**)&data, NEXT_MULT_OF_16(nzyxdim) * sizeof(T));
		 ...
		 */
		virtual void alias(const MultidimArray<T> &m)
		{
			copyShape(m);
			this->data=m.data;
		 ...
		 * @endcode
		 */
		virtual void resize(long int Ndim, long int Zdim, long int Ydim, long int Xdim)
		{
...
//#pragma omp parallel for
		 ...
		 * @endcode
		 */
		virtual void resize(long int Zdim, long int Ydim, long int Xdim)
		{
			resize(1, Zdim, Ydim, Xdim);
		}
		 ...
		 * @endcode
		 */
		virtual void resize(long int Ydim, long int Xdim)
		{
			resize(1, 1, Ydim, Xdim);
		}
		 ...
		 * @endcode
		 */
		virtual void resize(long int Xdim)
		{
			resize(1, 1, 1, Xdim);
		}
